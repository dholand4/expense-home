...
import {
  ...
  CloseButton, FieldLabel, ModalCard, ModalOverlay, ModalTitle, TitleRow, WarningText,
...
interface IProps {
  ...
  onClose: () => void;
  onSubmit: (id: string, data: Partial<IExpense>) => Promise<void>;
}
...
export function expenseEditFormGlobal({ visible, expense, onClose, onSubmit }: IProps) {
  ...
  useEffect(() => {
    if (expense) {
      ...
      setValue('total_amount', String(expense.total_amount));
  ...
  const onFormSubmit = async (data: IFormData) => {
    ...
    try {
      ...
      Alert.alert('Erro', 'Não foi possível editar o lançamento.');
    } finally {
