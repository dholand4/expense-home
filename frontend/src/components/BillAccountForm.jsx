...
import { createBillAccount } from '../services/billAccountService';

export default function BillAccountForm({ open, onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    await createBillAccount({ ...form, due_day: form.due_day ? parseInt(form.due_day) : undefined });
    setForm({ name: '', description: '', due_day: '' });
    setLoading(false);
    onSuccess();
    onClose();
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="bg-card border-border">
        <DialogHeader>
          <DialogTitle className="text-foreground">Nova Conta Avulsa</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="Ex: João, Condomínio..."
              required
